...
- the subscription allows 2 concurrent generations; a third concurrent call
  ...
  sharing one key can still trip the API limit.
"""
...
class _RequestFailed(RuntimeError):
    """A non-content HTTP failure; carries the status so 429s can be retried."""
...
class ElevenLabsRenderer:
    ...
    def render(
        ...
        with _semaphore:
            try:
                ...
                # One retry for transient rate limiting only; MusicPromptError
                # is content, not weather, and propagates untouched.
    ...
    def _post(self, body: bytes) -> tuple[bytes, dict[str, Any]]:
        ...
        try:
            ...
            raise RuntimeError(f"ElevenLabs music request failed: {e.reason}") from e
