...
- "contact" / "social": a player hears its own previous round AND the other
  ...
  hash, and — when the runner measured one — a `heard` dict of what that
  take actually SOUNDED like, from `afar.perception.ear`). Round t-1 only —
  round-t material does not exist when the context is built, and the loop
  must never close within a round.
...
def hears_others(condition: str) -> bool:
    """Whether this condition lets a player hear the other acts at all —
    the one question the runner may ask so it only measures hearing (the
    per-take `ear` pass) for sets where someone will actually hear it. What
    is HEARD still branches only inside `build_context`."""
    return condition in _CONTACT_CONDITIONS
...
class RoundEntry:
    ...
    `heard_by` is the measured hearing (afar.perception.ear): listener
    player_id -> the heard dict of what THIS take sounded like from that
    listener's seat (the relations are listener-relative, so there is one per
    listener). It is runner-computed residue, keyed by who may receive it —
    `to_context` never emits it; only `build_context` hands each listener
    their own, and only in contact conditions. A player never receives the
    measured facts of its OWN take: they made it.
    """
    ...
    heard_by: Mapping[str, dict[str, Any]] = field(default_factory=dict)
...
def build_context(
    ...
    if condition in _CONTACT_CONDITIONS:
        ...
        for entry in run.others(player_id, t - 1):
            item = entry.to_context()
            heard = (entry.heard_by or {}).get(player_id)
            if heard is not None:
                # Measured hearing: what that take SOUNDED like from this
                # listener's seat — contact-only, like everything in others.
                item["heard"] = dict(heard)
            others.append(item)
